---
title: "Data Analysis for \"Incremental View Model Synchronization Using Partial Models\" with Short Logs"
...
This report contains data analysis for the papaer "Incremental View Model Synchronization Using Partial Models" using a subset of the experiment configurations.
...
After extracting the benchmarking environment for the appropriate operating system, the benchmark configuration `short.json` can be run as follows:

    ./eclipse -benchmarks short.json -vmargs -Xmx8g

The results are places into `./results/short/benchmarks.log`, which should be copied into the same folder as this `.Rmd` document under the name `short_log.csv` before knitting.
